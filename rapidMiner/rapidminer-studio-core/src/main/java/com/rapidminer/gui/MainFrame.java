...
import io.transwarp.midas.MidasUnitExecutionFactory;
import io.transwarp.midas.ui.actions.ManageCustomOpAction;
import io.transwarp.midas.ui.RemoteResultDisplay;
import io.transwarp.midas.ui.actions.RendersAction;
import io.transwarp.midas.ui.actions.ScheduleAction;
...
public class MainFrame extends ApplicationFrame implements WindowListener {
	...
	private static String getModeString() {
		...
		if (OperatorService.isRemoteMode()) {
			return I18N.getMessage(I18N.getGUIBundle(), RapidMiner.MIDAS_GUI_MODE_REMOTE);
		} else if (OperatorService.isLocalMode()) {
			return I18N.getMessage(I18N.getGUIBundle(), RapidMiner.MIDAS_GUI_MODE_LOCAL);
		} else {
	...
	public final transient ScheduleAction SCHEDULE_ACTION = new ScheduleAction(this);
	...
	public final transient Action LOGIN_ACTION = new LoginAction();
	public final transient Action LOGOUT_ACTION = new LogoutAction();
	public final transient Action RENDERS_ACTION = new RendersAction();
	public final transient Action MANAGE_CUSTOM_OP_ACTION = new ManageCustomOpAction();
	...
	private final RemoteResultDisplay remoteDisplay = new RemoteResultDisplay();
	...
	public void validateProcess(final boolean force) {
		if (force || process.getProcessState() != Process.PROCESS_STATE_RUNNING) {
			List<ExecutionUnit> units = getProcess().getRootOperator().getSubprocesses();
			boolean remote = false;
			for (ExecutionUnit unit : units) {
				remote = MidasUnitExecutionFactory.getInstance().getExecutor(unit).isRemote(unit);
				if (remote) {
					metaDataUpdateQueue.validateRemote(unit);
					break;
				}
			}
			if (!remote) {
	...
	public MainFrame() {
		...
		if (SystemInfoUtilities.getOperatingSystem() == OperatingSystem.OSX) {
			...
			try {
				...
				OSXAdapter.adaptUI(this, RENDERS_ACTION, new AboutAction(this), quitListener);
			} catch (Throwable t) {
		...
		addProcessEditor(remoteDisplay);
		...
		if (OperatorService.isLocalMode()){
			...
			dockingDesktop.registerDockable(remoteDisplay);
		} else {
			throw new IllegalArgumentException("illegal client mode");
		}
		...
		if (OperatorService.isLocalMode()){
			...
			remoteDisplay.getDockKey().setCloseEnabled(false);
			remoteDisplay.getDockKey().setAutoHideEnabled(false);
			remoteDisplay.init(this);
		} else {
			throw new IllegalArgumentException("illegal client mode");
		}
		// menu bar
		...
		processMenu.add(SCHEDULE_ACTION);
	...
	public void finishInitialization() {
		...
		if (SystemInfoUtilities.getOperatingSystem() != OperatingSystem.OSX || !OSXAdapter.isAdapted()) {
			...
			settingsMenu.add(RENDERS_ACTION);
			settingsMenu.add(MANAGE_CUSTOM_OP_ACTION);
			settingsMenu.add(LOGIN_ACTION);
			settingsMenu.add(LOGOUT_ACTION);
		}
	...
	public RemoteResultDisplay getRemoteResultDisplay() {
		return remoteDisplay;
	}
		...
		try {
			synchronized (process) {
				...
				if (selectedOperator != null) {
					Operator restoredOperator = getProcess().getRootOperator();
				...
				if (shownOperatorChain != null) {
					OperatorChain restoredOperatorChain = getProcess().getRootOperator();
