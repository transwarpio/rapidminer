...
import io.transwarp.midas.constant.midas.params.ProcessParams;
...
public final class ProcessRootOperator extends OperatorChain {
	...
	private static final OperatorVersion OPERATOR_REPLACE_MACROS_CAUSES_ERROR_ON_UNDEFINED = new OperatorVersion(1, 0, 0);
	...
	public List<ParameterType> getParameterTypes() {
		...
		ParameterType cron = new ParameterTypeCronExpression(ProcessParams.CronExpr(), "cron expression", true, false);
		types.add(cron);

		ParameterType globalVars = new ParameterTypeList(ProcessParams.GlobalVars(), "global variables",
				new ParameterTypeString("name", "variable name"),
				new ParameterTypeString("value", "variable value"), false);
		types.add(globalVars);

		types.add(new ParameterTypeInt(
				ProcessParams.MaxRows(), "max result dataset rows",
