...
import com.rapidminer.gui.operatortree.actions.DeleteUserDefinedAction;
...
public class NewOperatorGroupTree extends JPanel implements FilterListener, SelectionNavigationListener {
	...
	private Operator getSelectedOperator() {
		...
		if (selectedOperator != null && selectedOperator instanceof OperatorDescription) {
			try {
				if (UserDefinedOperatorService.isUserDefined((OperatorDescription)selectedOperator)) {
					return ((UserDefinedOp)((OperatorDescription)selectedOperator)
							.createOperatorInstance()).getSubProcess();
				} else {
