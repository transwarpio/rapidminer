...
public abstract class Operator extends AbstractObservable<Operator>
		implements ConfigurationListener, PreviewListener, LoggingHandler, ParameterHandler, ResourceConsumer, IOperator {

	// this is to distinguish rapidminer local op and midas op
	// our own operator should override this variable as true
	...
	private static final OperatorVersion THROW_ERROR_ON_UNDEFINED_MACRO = new OperatorVersion(1, 0, 0);
	 ...
	 *
	 * @param The key of the user data.
	...
	public String getMidasXML(boolean hideDefault) {
		return getMidasXML(hideDefault, false);
	}

	@Override
	public String getMidasJson() {
		return new MidasJsonExporter().export(this);
	}
	/**
	...
	public String getMidasXML(boolean hideDefault, boolean onlyCoreElements) {
		try {
			return XMLTools.toString(new io.transwarp.midas.MidasXMLExporter(onlyCoreElements).exportProcess(this, hideDefault));
		} catch (Exception e) {
			LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
							"com.rapidminer.operator.Operator.generating_xml_process_error", e), e);
			return e.toString();
	...
	final public void setEnclosingProcess(ExecutionUnit parent) {
