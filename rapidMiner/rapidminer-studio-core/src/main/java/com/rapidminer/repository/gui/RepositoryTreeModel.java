...
public class RepositoryTreeModel implements TreeModel {
	...
	public Object getChild(Object parent, int index) {
		if (parent instanceof RepositoryManager) {
			...
			if (folder.willBlock()) {
				...
				try {
					List<Folder> subFolders = getVisibleSubFolders(folder);
					int numFolders = subFolders.size();
					if (index < numFolders) {
						return subFolders.get(index);
					} else if (onlyFolders) {
						return null;
					} else {
						List<DataEntry> dataEntries = getVisibleEntries(folder);
	...
	public int getChildCount(Object parent) {
		if (parent instanceof RepositoryManager) {
			...
			if (folder.willBlock()) {
				...
				try {
					List<Folder> subs = getVisibleSubFolders(folder);
					if (onlyFolders) {
						return subs.size();
					} else {
						return subs.size() + getVisibleEntries(folder).size();
					}
				} catch (RepositoryException e) {
					LogService
							.getRoot()
		...
		if (parent instanceof RepositoryManager) {
			...
			try {
				...
				if (child instanceof Folder) {
					return subs.indexOf(child);
				} else if (child instanceof Entry && !onlyFolders) {
					return getVisibleEntries(folder).indexOf(child) + subs.size();
				} else {
					return -1;
				}
			} catch (RepositoryException e) {
	...
	private boolean isEntryVisible(DataEntry entry) {
		if (hasFilter()) {
			return entry.getName().contains(getFilter());
		} else {
			return true;
		}
	}

	private List<DataEntry> getVisibleEntries(Folder folder) throws RepositoryException {
		if (folderBlocking(folder)) {
			unblock(folder);
			return new ArrayList<>();
		}
		if (!hasFilter()) {
			return folder.getDataEntries();
		}
	...
	private List<Folder> getVisibleSubFolders(Folder folder) throws RepositoryException {
		if (folderBlocking(folder)) {
			unblock(folder);
	...
	// fold is visible only if it has visible entries
	private boolean isFolderVisible(Folder folder) throws RepositoryException {
	...
	private boolean folderBlocking(Folder folder) {
		return folder.willBlock() || pendingFolders.contains(folder);
	}
}
