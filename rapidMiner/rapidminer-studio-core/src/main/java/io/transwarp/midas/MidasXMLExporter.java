...
public class MidasXMLExporter {
  ...
  public static final String URL ="http://transwarp.io/2016/midas";
  private boolean onlyCoreElements = false;

  public MidasXMLExporter() {
    this(false);
  }
   ...
   */
  public MidasXMLExporter(boolean onlyCoreElements) {
  ...
  public Document exportProcess(Operator operator, boolean hideDefault) throws IOException {
    try {
      ...
      requestElement.setAttribute(XMLNS, URL);
  ...
  public void appendXML(Parameters parameters, Element toElement, boolean hideDefault, Document
          doc) {
    for (String key : parameters.getKeys()) {
      ...
      if (type != null) {
        paramElement = type.getXMLForServer(key, value, hideDefault, doc);
      } else {
