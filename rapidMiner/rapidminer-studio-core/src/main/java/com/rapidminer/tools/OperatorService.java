...
import io.transwarp.midas.util.LocaleUtil;
...
public class OperatorService {
	...
	public static final String RAPID_MINER_CORE_PREFIX = "Midas Core";
	public static final String RAPID_MINER_CORE_NAMESPACE = "core";
	public static String OPERATORS_XML = "OperatorsCore.xml";
	private static String OPERATORS_XML_REMOTE = "MidasOperatorsCore.xml";

	private static String EN_SUFFIX = "";
	private static String ZH_SUFFIX = "_zh";
	private static String SUFFIX = EN_SUFFIX;


	static {
		if(LocaleUtil.getLocale().getLanguage().equals(Locale.CHINA.getLanguage()) ){
			SUFFIX = ZH_SUFFIX;
		}

	}
	private static final String clientMode =
			ParameterService.getParameterValue(RapidMiner.MIDAS_MODE).toLowerCase();

	private static final String rapidGUITestMode =
			ParameterService.getParameterValue(
					RapidMiner.PROPERTY_RAPIDMINER_GENERAL_RAPIDTESTMODE).toLowerCase();

	private static final Set<String> modeKeys = new HashSet<>();

	static {
		modeKeys.add(RapidMiner.Mode.LOCAL.toString().toLowerCase());
		modeKeys.add(RapidMiner.Mode.REMOTE.toString().toLowerCase());
	}

	public static boolean isLocalMode() {
		return clientMode.equals(RapidMiner.Mode.LOCAL.toString().toLowerCase());
	}

	public static boolean isRemoteMode() {
		return clientMode.equals(RapidMiner.Mode.REMOTE.toString().toLowerCase());
	}

	public static boolean isRapidGUITestMode() {
		return rapidGUITestMode.equals("true");
	}
	...
	public static void init() {
		...
		URL remoteOperators = getRemoteOperators();
		if (remoteOperators == null) {
			LogService.getRoot().log(Level.SEVERE,
					"com.rapidminer.tools.OperatorService.main_operator_descripton_file_not_found",
					new Object[] { Tools.RESOURCE_PREFIX, OPERATORS_XML_REMOTE });
		} else {
			registerOperators(remoteOperators, null, null);
		}
	...
	private static void parseOperators(Document document, ClassLoader classLoader, Plugin provider)
		...
		String docBundle = document.getDocumentElement().getAttribute("docbundle") + SUFFIX;
		...
		for (int i = 0; i < children.getLength(); i++) {
			...
			if (child instanceof Element) {
				...
				if (childElement.getTagName().equals("group")) {
					...
					if (modeKeys.contains(name) && !name.equals(clientMode.toLowerCase())) {
						// group name is "local" or "remote", but not match current client mode,
						// do nothing
					} else {
	...
	/** Returns the main operator description file (XML). */
	private static URL getRemoteOperators() {
		String resource;
		String operatorsXML = System.getProperty(RapidMiner.PROPERTY_RAPIDMINER_INIT_OPERATORS);
		if (operatorsXML != null) {
			resource = operatorsXML;
			...
			resource = "/" + Tools.RESOURCE_PREFIX + OPERATORS_XML_REMOTE;
		}
		return OperatorService.class.getResource(resource);
	}
