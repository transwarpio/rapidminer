...
public class SingleResultOverview extends JPanel {
				...
				public void run() {
					try {
						...
						if (entry instanceof IOObjectEntry) {
							...
							if (remote) {
								RapidMinerGUI.getMainFrame().getRemoteResultDisplay().showResultWithName(result, tabName);
							} else {
								RapidMinerGUI.getMainFrame().getResultDisplay().showResultWithName(result, tabName);
							}
						} else {
	...
	public SingleResultOverview(IOObject result, Process process, final String tabName, int resultIndex) {
		...
		String name = "Result: " + Integer.toString(resultIndex + 1);
		if (result instanceof ExampleSet) {
			resultName = ((ResultObject) result).getName();
