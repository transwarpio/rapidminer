 ...
 * @see PCA
 */
public class PCAModel extends AbstractEigenvectorModel implements ComponentWeightsCreatable, IPCAModel {
	...
	// this constructor is for midas
	public PCAModel(ExampleSet set, String[] attributeNames, double[] eigenvalues, double[][] eigenvectors) {
		super(set);
		this.attributeNames = attributeNames;

		this.eigenVectors = new ArrayList<>(eigenvalues.length);
		for (int i = 0; i < eigenvalues.length; i++) {
			double[] currentEigenVector = new double[attributeNames.length];
