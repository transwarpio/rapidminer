...
import com.rapidminer.repository.local.SampleRepository;
import com.rapidminer.repository.resource.ResourceRepository;
import com.rapidminer.tools.*;
import com.rapidminer.tools.Observer;
import com.rapidminer.tools.plugin.Plugin;
import io.transwarp.midas.remoteImpl.RemoteRepositoryImpl;
...
public class RepositoryManager extends AbstractObservable<Repository> {
	...
	private static final String RESOURCES_LOCATION_EN = "samples";

	private static String RESOURCES_LOCATION = RESOURCES_LOCATION_EN;

	private RepositoryManager() {
		if (sampleRepository == null) {
			if (OperatorService.isLocalMode()) {
				sampleRepository = new ResourceRepository(SAMPLE_REPOSITORY_NAME, RESOURCES_LOCATION);
				repositories.add(sampleRepository);
			} else if (OperatorService.isRemoteMode()) {
				File sampleDir = FileSystemService.getRemoteSampleDir();
				if (sampleDir.exists()) {
					try {
						sampleRepository = new SampleRepository(SAMPLE_REPOSITORY_NAME, sampleDir);
						repositories.add(sampleRepository);
		...
		addRepository(new RemoteRepositoryImpl());
		sortRepositories();
	...
	public void createRepositoryIfNoneIsDefined() {
		...
		if (noLocalRepository) {
			try {
				LocalRepository defaultRepo = new LocalRepository(LOCAL_REPOSITORY);
