...
public class SimpleProcessEntry extends SimpleDataEntry implements ProcessEntry {
	...
	public void storeMidasXMLAndJson(Operator operator) throws RepositoryException {
		try {
			String xmlFile = getFile().getAbsolutePath();
			int index = xmlFile.lastIndexOf(".");
			String prefix = xmlFile.substring(0, index);

//			// xml
//			String xml = operator.getMidasXML(false);
//			Tools.writeTextFile(new File( prefix + ".xml"), xml);

			// json
            String json = operator.getMidasJson();
            Tools.writeTextFile(new File( prefix + ".json"), json);
		} catch (Exception e) {
			throw new RepositoryException("Cannot write " + getFile() + ": " + e, e);
		}
	}
