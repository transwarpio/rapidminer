...
import io.transwarp.midas.adaptor.IInputPorts;
 ...
 */
public class ExecutionUnit extends AbstractObservable<ExecutionUnit> implements IExecutionUnit {
	...
	@Override
	public IInputPorts getIInnerSinks() {
		return innerInputPorts;
	}

	@Override
	public IOperator getIEnclosingOperator() {
		return enclosingOperator;
	}

	@Override
	public Enumeration<IOperator> getIOperatorEnumeration() {
		// should be a single mapping in scala, ugly in java
		Vector<IOperator> iops = new Vector<IOperator>();
		for( Operator op: operators){
			iops.add(op);
		}
		return iops.elements();
	}
