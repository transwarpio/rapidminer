...
public class SettingsPropertyPanel extends PropertyPanel {
    ...
    public void applyProperties() {
        for (ParameterType type : shownParameterTypes) {
            ...
            if (ParameterService.PARAMETER_CHECK.contains(type.getKey())) {
                if (!(value.equals(ParameterService.getParameterValue(type.getKey()))))
                    SwingTools.showMessageDialog("please_restart_system",
                            I18N.getMessage(I18N.getSettingsBundle(), type.getKey() + ".title"));
            }
