	...
	public void store(Process process, ProgressListener listener) throws IOException {
		try {
			...
			if (entry == null) {
				...
				entry = repositoryLocation.locateEntry();
				((ProcessEntry) entry).storeMidasXMLAndJson(process.getRootOperator());
			} else {
				if (entry instanceof ProcessEntry) {
					...
					if (isReadOnly) {
						...
						UserData updateRevisionOnSave = process.getRootOperator().getUserData(UPDATE_REVISION_ON_SAVE_KEY);
