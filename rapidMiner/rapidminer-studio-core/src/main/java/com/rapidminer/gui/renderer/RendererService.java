...
public class RendererService {
	...
	private static Map<String, Set<String>> blackList = new HashMap<>();
	...
	public static void init() {
		...
		try {
			...
			Map<String, Set<String>> blackListObj = (Map<String, Set<String>>)objectOutputStream.readObject();
			setBlackList(blackListObj);
		} catch (IOException e) {
			LogService
					.getRoot()
					.log(Level.WARNING,
							"can't find renderer.sav");
	...
	public static Map<String, Set<String>> getBlackList() {
		...
		for (Entry<String, Set<String>> entry: blackList.entrySet()) {
	...
	public static void setBlackList(Map<String, Set<String>> blackList_) {
		blackList = blackList_;
	}

	private static boolean inBlackList(String renderableObjectName,
								String renderName) {
		if (blackList.containsKey(renderableObjectName)) {
			if (blackList.get(renderableObjectName).contains(renderName)) {
				return true;
			} else {
				return false;
			}
	...
	public static Collection<Renderer> blackListFilter(String renderableObjectName,
													   Collection<Renderer> renders) {
		if (blackList.containsKey(renderableObjectName)) {
			Set<Renderer> result = new HashSet<>();
			for (Renderer renderer : renders) {
				if (!blackList.get(renderableObjectName).contains(renderer.getName())) {
					result.add(renderer);
				}
			}
			return result;
